...
resource "google_service_account" "backend_sa" {
  account_id   = "${var.project_name}-${var.environment}-backend-sa"
  display_name = "PullSense ${var.environment} Backend Cloud Run Service Account"
  project      = var.project_id
}

# Frontend Cloud Run Service Account
resource "google_service_account" "frontend_sa" {
  account_id   = "${var.project_name}-${var.environment}-frontend-sa"
  display_name = "PullSense ${var.environment} Frontend Cloud Run Service Account"
  project      = var.project_id
}

# GitHub Actions CD Service Account
resource "google_service_account" "github_cd_sa" {
  account_id   = "${var.project_name}-${var.environment}-gh-cd"
  display_name = "PullSense ${var.environment} GitHub Actions CD Service Account"
  project      = var.project_id
}

# Backend IAM Role Grants
locals {
  backend_roles = [
...
resource "google_iam_workload_identity_pool" "github_pool" {
  ...
  workload_identity_pool_id = "${var.project_name}-${var.environment}-gh-pool"
  display_name              = "PullSense GitHub Actions OIDC Pool"
  project                   = var.project_id
}
